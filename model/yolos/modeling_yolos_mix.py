...
class YolosForObjectDetection(YolosPreTrainedModel):
    def __init__(self, config: YolosConfig):
        ...
        self.det_text_attn = nn.MultiheadAttention(
            embed_dim=config.hidden_size,
            num_heads=config.num_attention_heads,
            batch_first=True,
        )
        self.det_text_ln = nn.LayerNorm(config.hidden_size)
        ...
        self.cross_ln = nn.LayerNorm(config.hidden_size)
        
        self.det_proj = nn.Linear(4096, config.hidden_size)
        self.text_proj = nn.Linear(4096, config.hidden_size)
    ...
    def forward(
        ...
        det_feats: Optional[Tensor]=None,               ############## new
        text_feats: Optional[Tensor]=None,               ############## new
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        det_feats = self.det_proj(det_feats)
        text_feats = self.text_proj(text_feats)
        
        det2text, _ = self.det_text_attn(
            query=det_feats,    # your detection tokens
            key=text_feats,       # text_feature as key
            value=text_feats,     # text_feature as value
        )
        ext_feats = self.det_text_ln(det_feats + det2text)
        
        #print('ext_feats', ext_feats)
        
        # 3) cross-attend detection tokens (queries) to text features (keys/values)
        #if ext_feats is not None:

        cross_out, _ = self.cross_attn(
            query=sequence_output,    # your detection tokens
            key=ext_feats,       # text_feature as key
            value=ext_feats,     # text_feature as value
        )
        sequence_output = self.cross_ln(sequence_output + cross_out)
